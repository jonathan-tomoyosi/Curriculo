...
export async function GET(_request: Request, { params }: { params: Promise<{ locale: string }> }) {
  ...
  try {
    ...
    // empacotamento, não de código, e nenhum teste local a alcançava. O log guarda o
    // motivo real; a resposta continua genérica para não expor detalhe interno a quem só
    // pediu um currículo. A prevenção agora vive em scripts/verify-trace.mjs, que roda
    // dentro do build.
